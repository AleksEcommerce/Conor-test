document.addEventListener('DOMContentLoaded', function() {
  ...
  spacersCheckboxes.forEach((checkbox, index) => {
    ...
    checkbox.addEventListener('click', function() {
      // const productData = getProductFromLocalStorage(this.getAttribute('data-product-id'));
      const getThirdProducts = JSON.parse(localStorage.getItem('builder.thirdStepProducts'));
      if (getThirdProducts.length < maxCharms) {
        const productData = getProductFromLocalStorage(this.getAttribute('data-product-id'));
          getThirdProducts.push(productData);
          localStorage.setItem('builder.thirdStepProducts', JSON.stringify(getThirdProducts));
          ...
          const newCharmImage = document.createElement('img');
          newCharmImage.setAttribute('src', spacerImageSrc);
          newCharmImage.classList.add('b-charms_presentation--item_img');
          newCharmItem.appendChild(newCharmImage);
          spacerList.appendChild(newCharmItem);
          ...
          counterSpacers++;
          maxSpacers = counterCharms - 1;
          updateCharmListClass(getThirdProducts, spacerList, maxCharms); // Передаем правильный контейнер
          checkStep();
      } 
